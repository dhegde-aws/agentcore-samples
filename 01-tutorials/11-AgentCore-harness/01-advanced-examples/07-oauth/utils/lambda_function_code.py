...
# Mock order database
ORDERS = {
    "ORD-001": {"orderId": "ORD-001", "item": "Mechanical Keyboard", "status": "shipped", "amount": 149.99},
    "ORD-002": {"orderId": "ORD-002", "item": "USB-C Hub", "status": "processing", "amount": 59.99},
    "ORD-003": {"orderId": "ORD-003", "item": "Monitor Stand", "status": "delivered", "amount": 89.99},
}
...
def lambda_handler(event, context):
    ...
    if tool_name == "get_order":
        ...
        if order:
            ...
            return {"status": "success", "result": json.dumps({"orderId": order_id, "newStatus": new_status})}
        return {"status": "error", "result": f"Order {order_id} not found"}
