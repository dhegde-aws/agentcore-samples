...
import boto3, json, time, uuid, zipfile, io
...
def create_m2m_pool(region: str, prefix: str) -> dict:
    ...
    try:
        cog.create_resource_server(
            UserPoolId=pool_id, Identifier=rs_id,
    ...
    return dict(
        ...
        discovery_url=discovery_url, scope=scope,
        domain_prefix=domain_prefix, token_endpoint=token_endpoint,
    )
...
def create_credential_provider(
    region: str, prefix: str,
    discovery_url: str, client_id: str, client_secret: str,
) -> dict:
...
def deploy_lambda(region: str, prefix: str) -> dict:
    ...
    try:
        ...
        fn_arn = lam.create_function(
            ...
            Description='Order management for AgentCore Gateway', Timeout=30,
        )['FunctionArn']
...
def create_gateway_with_lambda_target(
    region: str, prefix: str, account_id: str,
    discovery_url: str, allowed_client: str, allowed_scope: str,
    lambda_arn: str, lambda_function_name: str,
) -> dict:
    ...
    try:
        ...
        gw = ac.create_gateway(
            name=gateway_name, protocolType='MCP', roleArn=gw_role_arn,
    ...
    return dict(
        gateway_id=gw_id, gateway_arn=gw_full.get('gatewayArn', gw_arn),
        gateway_url=gw_full.get('gatewayUrl', gw_url),
        target_id=tgt_id, role_name=gw_role_name, policy_name=gw_policy_name,
    )
...
def cleanup_all(region: str, prefix: str):
    ...
    deleted, skipped = [], []
    def ok(m):  deleted.append(m);  print(f'  ✓ {m}')
    def skip(m): skipped.append(m); print(f'  – {m}')

    def _wait_gone(check_fn, retries=24, delay=5, **kwargs):
        for _ in range(retries):
            try: check_fn(**kwargs); time.sleep(delay)
            except Exception: return

    # 1. Harness
    print('\n[1/7] Harness')
    try:
        matches = [h for h in ac.list_harnesses().get('harnesses', [])
    ...
    print('\n[2/7] Gateway + targets')
    try:
        gws = [g for g in ac.list_gateways().get('items', [])
    ...
    try:
        ...
        skip('Credential provider not found' if 'not found' in str(e).lower()
             or 'ResourceNotFound' in str(type(e).__name__) else f'{e}')
    ...
    def _del_role(rname, policy_names=None):
        try:
            ...
            skip(f'Role {rname} not found'); return
        except Exception as e:
            skip(f'{rname}: {e}'); return
        try:
        ...
        for pn in (policy_names or []):
            try:
                ...
                pass
        try:
            iam_c.delete_role(RoleName=rname); ok(f'Role {rname} deleted')
        except Exception as e:
            skip(f'Role {rname}: {e}')
    ...
    print('\n[6/7] Cognito Pool #2')
    try:
        m = [p for p in cog.list_user_pools(MaxResults=60)['UserPools']
        ...
        if m:
            ...
            try:
                ...
                pass
            cog.delete_user_pool(UserPoolId=pid); ok(f'Pool #2 {pid} deleted')
        else:
    ...
    try:
        ...
        if m:
            pid = m[0]['Id']
            cog.delete_user_pool(UserPoolId=pid); ok(f'Pool #1 {pid} deleted')
        else:
